...
static BOOL buffer_alloc_persistent_map(struct wined3d_buffer *buffer, struct wined3d_context *context)
{
    ...
    if (buffer_gl->b.flags & WINED3D_BIND_CONSTANT_BUFFER)
    {
    ...
    buffer_gl->b.buffer_heap = heap;
    if (FAILED(hr = wined3d_buffer_heap_alloc(heap, buffer_gl->b.resource.size, &map_range)))
    {
        goto fail;
    }
    buffer_gl->b.cs_persistent_map = map_range;
    buffer_gl->b.mt_persistent_map = map_range;
    return TRUE;

fail:
    // FIXME(acomminos): fall back to standalone BO here?
    ERR("Failed to create persistent map for buffer %p, hr=%x\n", buffer_gl, hr);
    buffer_gl->b.buffer_heap = NULL;
    return FALSE;
}

static void buffer_free_persistent_map(struct wined3d_buffer_gl *buffer_gl)
{
    if (!buffer_gl->b.buffer_heap)
        return;

    // TODO(acomminos): get the CS thread to free pending main thread buffers.
    wined3d_buffer_heap_free(buffer_gl->b.buffer_heap, buffer_gl->b.cs_persistent_map);
    buffer_gl->b.buffer_heap = NULL;
}
...
static void buffer_unload(struct wined3d_resource *resource)
{
    ...
    buffer_free_persistent_map(wined3d_buffer_gl(buffer));
...
static void wined3d_buffer_gl_destroy_object(void *object)
{
    ...
    buffer_free_persistent_map(buffer_gl);
        ...
        struct wined3d_map_desc *map_desc, const struct wined3d_box *box, DWORD flags)
{
    ...
    if (buffer->locations & WINED3D_LOCATION_PERSISTENT_MAP)
    {
        ...
        if (buffer_gl->b.flags & WINED3D_MAP_DISCARD)
        {
            HRESULT hr;
            struct wined3d_map_range map_range;
            if (FAILED(hr = wined3d_buffer_heap_alloc(buffer_gl->b.buffer_heap, resource->size, &map_range)))
            {
