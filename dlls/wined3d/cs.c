...
static void wined3d_cs_exec_discard_buffer(struct wined3d_cs *cs, const void *data)
{
    ...
    if (FAILED(hr = wined3d_buffer_heap_free_fenced(wined3d_buffer_gl(buffer)->b.buffer_heap, cs->device, wined3d_buffer_gl(buffer)->b.cs_persistent_map)))
    {
        ERR("Failed to do a fenced free on discarded buffer %p, hr %x\n. Freeing anyway.", buffer, hr);
        wined3d_buffer_heap_free(wined3d_buffer_gl(buffer)->b.buffer_heap, wined3d_buffer_gl(buffer)->b.cs_persistent_map);
    }

    wined3d_buffer_gl(buffer)->b.cs_persistent_map = op->map_range;

    // TODO(acomminos): merge this logic with buffer.c functions for standalone BOs
    if (wined3d_buffer_gl(buffer)->b.flags & WINED3D_BIND_VERTEX_BUFFER)
        device_invalidate_state(cs->device, STATE_STREAMSRC);
    if (wined3d_buffer_gl(buffer)->b.flags & WINED3D_BIND_INDEX_BUFFER)
        device_invalidate_state(cs->device, STATE_INDEXBUFFER);
    if (wined3d_buffer_gl(buffer)->b.flags & WINED3D_BIND_CONSTANT_BUFFER)
    {
